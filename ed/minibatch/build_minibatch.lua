...
function parse_candidate_entities(parts, type_parts, for_training, orig_max_num_cand)
  ...
  for cand_index = 1, math.min(num_cand, orig_max_num_cand) do 
    ...
    ent_type_id[cand_index] = tostring(cand_type_id)
    assert(#ent_type_id[cand_index] == 8 * 3, "[ERROR]" .. '\t'.. parts[1])
...
function get_ent_type_vec(minibatch_type_id)
  ...
  for i = 1, r do
    for j = 1, c do
      -- print(minibatch_type_id[i][j])
      -- print(i .. '\t' .. j .. '\t' .. minibatch_type_id[i][j])
...
function minibatch_table2tds(inputs_table)
  ...
  inputs[4] = tds.Hash()
  inputs[4][1] = inputs_table[4][1]
  -- inputs[5] = tds.Hash()
  inputs[5] = {}
  inputs[5][1] = inputs_table[5][1]
  return inputs
end
